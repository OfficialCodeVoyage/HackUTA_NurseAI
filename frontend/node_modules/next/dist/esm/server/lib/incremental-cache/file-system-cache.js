   ...
   */ loadTagsManifestSync() {
        ...
        if (!data && process.env.NEXT_RUNTIME !== 'edge') {
            ...
            try {
                ...
                if (kind === 'fetch' && this.flushToDisk) {
                    ...
                    if (isAppPath) {
                        const rscData = await this.fs.readFile(this.getFilePath(`${key}${isRoutePPREnabled ? RSC_PREFETCH_SUFFIX : RSC_SUFFIX}`, 'app'));
                        data = {
                            lastModified: mtime.getTime(),
                            value: {
                                kind: 'APP_PAGE',
                        ...
                        const pageData = JSON.parse(await this.fs.readFile(this.getFilePath(`${key}${NEXT_DATA_SUFFIX}`, 'pages'), 'utf8'));
                        data = {
                            lastModified: mtime.getTime(),
                            value: {
                                kind: 'PAGE',
    ...
    async set(...args) {
        ...
        if ((data == null ? void 0 : data.kind) === 'PAGE' || (data == null ? void 0 : data.kind) === 'APP_PAGE') {
            ...
            await this.fs.writeFile(this.getFilePath(`${key}${isAppPath ? ctx.isRoutePPREnabled ? RSC_PREFETCH_SUFFIX : RSC_SUFFIX : NEXT_DATA_SUFFIX}`, isAppPath ? 'app' : 'pages'), isAppPath ? data.rscData : JSON.stringify(data.pageData));
