...
async function exportAppImpl(dir, options, span) {
  ...
  Read more: https://nextjs.org/docs/messages/export-image-api`);
    ...
    const results = await Promise.all(filteredPaths.map(async (path)=>{
        ...
        const exportPage = pathMap._isAppDir ? options.exportAppPageWorker : options.exportPageWorker;
        if (!exportPage) {
        ...
        for(let attempt = 0; attempt < maxAttempts; attempt++){
            try {
                ...
                if (attempt >= maxAttempts - 1) {
                    ...
                    if (maxAttempts > 0) {
                        _log.info(`Failed to build ${pageKey} after ${maxAttempts} attempts.`);
                    }
                    // If prerenderEarlyExit is enabled, we'll exit the build immediately.
                    if (nextConfig.experimental.prerenderEarlyExit) {
                        throw new ExportError(`Export encountered an error on ${pageKey}, exiting the build.`);
                    } else {
                    ...
                    _log.info(`Failed to build ${pageKey} (attempt ${attempt + 1} of ${maxAttempts}). Retrying again shortly.`);
                    await new Promise((r)=>setTimeout(r, Math.random() * 500));
                }
            }
        }
        // if we eventually succeeded, remove the page from the failed attempts map
        if (failedExportAttemptsByPage.has(pageKey) && result && !('error' in result)) {
