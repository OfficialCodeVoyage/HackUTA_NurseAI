   ...
   */ async handleApiRequest(req, res, query, match) {
    ...
    makeRequestHandler() {
        ...
        return (req, res, parsedUrl)=>{
            ...
            const loggingFetchesConfig = logging == null ? void 0 : logging.fetches;
            ...
            if (this.renderOpts.dev) {
                const { blue, green, yellow, red, gray, white, bold } = require('../lib/picocolors');
                ...
                const reqCallback = ()=>{
                    ...
                    for(let i = 0; i < fetchMetrics.length; i++){
                        ...
                        if (enabledVerboseLogging) {
                            ...
                            writeStdoutLine(`${indentation}${white(metric.method)} ${white(url)} ${metric.status} in ${duration}ms ${status}`);
                            if (cacheReasonStr) {
                                writeStdoutLine(`${indentation}${indentation}${cacheReasonStr}`);
                            }
                            if (cacheWarning) {
                                writeStdoutLine(`${indentation}${indentation}${yellow(bold('⚠'))} ${white(cacheWarning)}`);
                            }
                        } else if (cacheWarning) {
                            writeStdoutLine(`${indentation}${white(metric.method)} ${white(url)}`);
                            writeStdoutLine(`${indentation}${indentation}${yellow(bold('⚠'))} ${white(cacheWarning)}`);
                        }
                    }
